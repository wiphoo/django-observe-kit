...
def register_user(api_url: str, email: str, password: str) -> bool:
    ...
    try:
        req = urllib.request.Request(register_url, data=payload, headers=headers, method="POST")
...
def verify_clickhouse_connection(api_url: str, session_token: Optional[str] = None) -> None:
    """Log ClickHouse datasource status. Best-effort."""
    ...
    try:
        req = urllib.request.Request(connections_url, headers=headers, method="GET")
